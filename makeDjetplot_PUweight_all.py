...
plotloc=''
# 50ns + 25ns silver 
pileup = [0.0, 0.5544735677695405, 0.6786295980341037, 1.0617233606832137, 1.3550441155701411, 1.5998389593590092, 1.9069816558364803, 1.4202736159977019, 1.2556667249726585, 1.349910859675518, 1.3307574519663052, 1.2290535576602792, 1.1321589618055525, 1.0349277283995624, 0.8902810729583754, 0.7018647868478741, 0.518259798658448, 0.39367207810926164, 0.3682628532877464, 0.4493225255126574, 0.5727185477397563, 0.6384212503880221, 0.630883328829914, 0.5902332966082422, 0.5345797312225625, 0.4660271427913152, 0.3886052191843434, 0.3101426352718437, 0.23807290247688265, 0.17686169855077677, 0.12793649081068006, 0.09071133394122137, 0.06355618920262626, 0.044454909890423645, 0.03141124581700232, 0.022683177595541143, 0.016880635431663966, 0.012969058619337104, 0.010221325809576463, 0.008158084961343253, 0.0064919882531231645, 0.005079099162042311, 0.003869406423068746, 0.002857951494392014, 0.002046391380436547, 0.001424107544153871, 0.0009665346324810172, 0.0006418863128730127, 0.00041824463892313756, 0.00026790984880305837, 0.00016893227596013886, 0.0]
...
class TreePlot(Plot):
    ...
    def vbf_eff_pu(self, bins=None):
        ...
        for bin in bins:
            ...
            eff_ct[bin] = 0.0
            eff_ct_err[bin] = 0.0
            n_pass_pu[bin] = [0.0 for i in range(9)] # n pass with weights
            n_fail_pu[bin] = [0.0 for i in range(9)] # n fail with weights s
            n_pass_pu_sumw2[bin] = [0.0 for i in range(9)] # n pass with weights 
            n_fail_pu_sumw2[bin] = [0.0 for i in range(9)] # n fail with weights 
            eff_pu[bin] = [0.0 for i in range(9)] # eff 
            eff_pu_err[bin] = [0.0 for i in range(9)] # eff errors 
        ...
        for i, entry in enumerate(t):
            ...
            # pileup weight, 5% PU weight uncertainties
            puwts = [0.0,0.0,0.0] # ct, up, dn
            puwts[0] = pileup[int(entry.NTrueInt)]
            puwts[1] = puwts[0]*(1.0+0.05)
            puwts[2] = puwts[0]*(1.0-0.05)
            ...
            for bin in bins:
                if bin.min < entry.ZZMass < bin.max:
                    if pass_ct:
                        ...
                        n_pass_ct_sumw2[bin] +=wt*wt
                        for ibb in range(3): 
                            n_pass_pu[bin][ibb] += wt*puwts[ibb]
                            n_pass_pu_sumw2[bin][ibb] += wt*puwts[ibb]*wt*puwts[ibb]
                    else:
                        n_fail_ct[bin] += wt
                        n_fail_ct_sumw2[bin] +=wt*wt
                        for ibb in range(3):
                            n_fail_pu[bin][ibb] += wt*puwts[ibb]
                            n_fail_pu_sumw2[bin][ibb] += wt*puwts[ibb]*wt*puwts[ibb]
        ...
        for bin in bins:
            ...
            for ibb in range(3):
                try:
                    eff_pu[bin][ibb] = n_pass_pu[bin][ibb]/(n_pass_pu[bin][ibb]+n_fail_pu[bin][ibb])
                    eff_pu_err[bin][ibb] = sqrt((n_fail_pu[bin][ibb]**2*n_pass_pu_sumw2[bin][ibb]+n_pass_pu[bin][ibb]**2*n_fail_pu_sumw2[bin][ibb])/(n_fail_pu[bin][ibb]**4))
                except ZeroDivisionError:
                    eff_pu[bin][ibb] = 0.0
                    eff_pu_err[bin][ibb] = 0.0
                    pass

        print '#####################################################'
        print 'bin       eff_ct[bin]    eff_pu[bin][0]   eff_ct_err[bin]   eff_pu_err[bin][0]  '        
        for i,bin in enumerate(bins):
            print ' '+str(i)+'  '+str(eff_ct[bin])+'  '+str(eff_pu[bin][0])+'  '+str(eff_ct_err[bin])+'  '+str(eff_pu_err[bin][0])
        print '#####################################################'

        print '#####################################################'
        print ' [bin][i] :     eff_pu[bin][i]     eff_pu_err[bin][i]  '
        for i,bin in enumerate(bins):
            for ib in range(3):
                print ' ['+str(i)+']['+str(ib)+']:  '+str(eff_pu[bin][ib])+'  '+str(eff_pu_err[bin][ib])
        print '#####################################################'

        return eff_ct,eff_ct_err,eff_pu,eff_pu_err
...
def makePUTable(massbins, outroot,  *plots):
    ...
    eff_md = collections.OrderedDict()
    eff_up = collections.OrderedDict()
    eff_dn = collections.OrderedDict()
    eff_unc_up = collections.OrderedDict()
    eff_unc_dn = collections.OrderedDict()
    ...
    for plot in plots:
        ...
        eff[plot] = collections.OrderedDict()
        eff_err[plot] = collections.OrderedDict()
        eff_md[plot] = collections.OrderedDict()
        eff_up[plot] = collections.OrderedDict()
        eff_dn[plot] = collections.OrderedDict()
        eff_unc_up[plot] = collections.OrderedDict()
        eff_unc_dn[plot] = collections.OrderedDict()
        ...
        eff_ct,eff_ct_err,eff_pu,eff_pu_err = plot.vbf_eff_pu(bins)

        for bin in bins:
            eff[plot][bin] = eff_ct[bin] # eff at center
            eff_err[plot][bin] = eff_ct_err[bin] # eff stats.error at  center
            eff_up[plot][bin] = max(eff_pu[bin])
            eff_dn[plot][bin] = min(eff_pu[bin])
            eff_md[plot][bin] = eff_pu[bin][0]
            #eff_unc_up[plot][bin] = abs(eff_up[plot][bin]-eff_md[plot][bin]) if eff_up[plot][bin]>0.0 else 0.0
            #eff_unc_dn[plot][bin] = abs(eff_dn[plot][bin]-eff_md[plot][bin]) if eff_dn[plot][bin]>0.0 else 0.0
            eff_unc_up[plot][bin] = 0.0 if eff_md[plot][bin]<=eff[plot][bin] else eff_md[plot][bin]-eff[plot][bin]
            eff_unc_dn[plot][bin] = 0.0 if eff_md[plot][bin]>=eff[plot][bin] else eff[plot][bin]-eff_md[plot][bin]
            if plot.title == "ttH" and bin.min >= 500: continue
            nbins[plot] += 1
            x[plot].append(bin.center)
            y[plot].append(eff[plot][bin])
...
#        if plot.title != "Z+X" : 
    ...
    legend.AddEntry(unc_g[plots[0]],"PU weight uncertainties", "f")
    ...
    pt.Draw()
    c1.Update()
    c1.SaveAs(plotloc+"VBFeffPUall.png")
    c1.SaveAs(plotloc+"VBFeffPUall.eps")
    c1.SaveAs(plotloc+"VBFeffPUall.root")
    c1.SaveAs(plotloc+"VBFeffPUall.pdf")
    ...
    pt.Draw()
    c2.Update()
    c2.SaveAs(plotloc+"VBFeffPU100all.png")
    c2.SaveAs(plotloc+"VBFeffPU100all.eps")
    c2.SaveAs(plotloc+"VBFeffPU100all.root")
    c2.SaveAs(plotloc+"VBFeffPU100all.pdf")
    ...
    print r"%%%%%%%%%%%%%% VBF-tag Efficiency with PU weight %%%%%%%%%%%%%%%"
    print r"\begin{center}"
    print r"\begin{tabular}{ |%s| }" % ("|".join("c" * (len(plots)+1)))
    ...
    for bin in bins:
        ...
        #print row_format.format('$\epsilon(up)$', *(eff_up[plot][bin]*100 for plot in plots))
        #print row_format.format('$\epsilon(dn)$', *(eff_dn[plot][bin]*100 for plot in plots))
        #print row_format.format('$\delta\epsilon(PU)/\epsilon$', *( (0.0 if eff[plot][bin]<=0.0 else abs(eff_up[plot][bin]-eff_dn[plot][bin])/2.0/eff_md[plot][bin]*100 ) for plot in plots))
        print row_format.format('$\epsilon(PU)$', *(eff_md[plot][bin]*100 for plot in plots))
        print row_format.format('$\delta\epsilon(PU)/\epsilon$', *( ( 0.0 if eff[plot][bin]<=0.0 else abs(eff_md[plot][bin]-eff[plot][bin])/2.0/eff[plot][bin]*100 ) for plot in plots))
    print r"\hline"
    print r"\end{tabular}"
    print r"\end{center}"
    print r"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
    ...
    #print "Process & $epsilon$ & $epsilon(up)$ & $epsilon(dn)$ & $\delta\epsilon(QCD)$ & $\delta\epsilon(QCD)/\epsilon$\\\\"
    #print "Process & $\epsilon$ & $\epsilon(up)$ & $\epsilon(dn)$ & $\delta\epsilon(QCD)/\epsilon$\\\\"
    print "Process & $\epsilon$ & $\epsilon(PU)$ & $\delta\epsilon(PU)/\epsilon$\\\\"
    for plot in plots:
        for bin in bins:
            #print plot.title,' & ','{0:.4f}'.format(eff[plot][bin]),' & ','{0:.4f}'.format(eff_up[plot][bin]),' & ','{0:.4f}'.format(eff_dn[plot][bin]),' & ','{0:.4f}'.format(( 0.0 if eff[plot][bin]<=0.0 else abs(eff_up[plot][bin]-eff_dn[plot][bin])/2/eff_md[plot][bin] )),' \\\\'
            print plot.title,' & ','{0:.4f}'.format(eff[plot][bin]),' & ','{0:.4f}'.format(eff_md[plot][bin]),' & ','{0:.4f}'.format(( 0.0 if eff[plot][bin]<=0.0 else abs(eff_md[plot][bin]-eff[plot][bin])/2/eff[plot][bin] )),' \\\\'
    print r"\hline"
    print r"\end{tabular}"
    print r"\end{center}"
    print r"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
...
if __name__ == "__main__":
    ...
    if forplot:
        ...
        makeDjetplots(*plots)
    elif fortable or doJEC or doQCD or doPU:
        plots = (
...
#            makeQCDTable([100, 110, 120, 130, 140, 150, 160, 170, 180, 190, 200, 300, 400, 500, 600, 700, 800, 900, 1000], *plots)
            ...
            makePUTable([100, 110, 120, 130, 140, 150, 160, 170, 180, 190, 200, 300, 400, 500, 600, 700, 800, 900, 1000], 'outpu_all.root', *plots)
            #makePUTable([100, 500],  'outpu.root', *plots)
